...
args = dict(
    seed=42,
    model='vit_base_patch16_clip_384',  
    lr=2e-5, #5e-5,
    weight_decay=0.001,
    log_dir='logs',
...
#         if self.hparams.args.loss == 'multilabel':
#             label = F.one_hot(label.long(), num_classes=self.hparams.args.num_classes) # to one_hot
        loss = self.criterion(pred, label)
...
def get_parameter_section(parameters, lr, wd):     
    ...
    for no, (n,p) in enumerate(parameters):  
        ...
        if any([(var in n) for var in freeze_layers]):
